...
| Logger (app-wide) | logging at every layer, see `04-LOGGING-AND-ERRORS.md` | 2-4 |
...
| Rich text editing | `react-quill-new`, `content_html` stored sanitized | 7 |
| Logging throughout the application | Pino instance imported at every layer | 2-4 |
| Log important events, errors, **http request and response**, user activities | `pino-http` req/res logging + the event table in `04-LOGGING-AND-ERRORS.md` | 2-4 |
...
| 2. Dashboard | list of user-specific notes, create-note button | fetch user notes, display list, navigate to editor | Part 7 (`Workspace.jsx`, `NoteList.jsx`, `NoteCard.jsx`) |
...
| Search and filter | **not deferred.** `?search=` is in the core API and the list pane | 4, 7 |
...
| "tasks" (in the optional-features wording) | notes | the brief's optional section reuses task wording from a template, but the domain here is notes |
| Profile screen "[optional]" | building it | it's small, and it satisfies "dashboard to display ... user profile" |
