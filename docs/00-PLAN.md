...
Incremental plan. Each part is independently runnable and verifiable. **Do not start a part until the previous one is verified.**
...
### Where we didn't take the obvious dependency

| Instead of | We use | Reason |
|---|---|---|
| `bcrypt` | `bcryptjs` | `bcrypt` is a native addon needing node-gyp and Visual Studio Build Tools on Windows, and a missing prebuild for Node 24 stalls setup. `bcryptjs` is pure JS, same API, hash-compatible. It is slower, which doesn't matter at this scale. |
| `react-quill` | `react-quill-new` | `react-quill` has had no release since **Sept 2023**, its peers cap at React 18, and it calls `findDOMNode`, which warns under React 18 StrictMode and is gone in React 19. `react-quill-new` is the maintained fork (React 16-19). |
| Express 5 | Express 4 | Express 5 auto-forwards rejected promises, which would remove `asyncHandler`. Staying on 4 keeps the middleware ecosystem (`express-rate-limit`, `helmet`) on its best-tested path, and keeps the error plumbing explicit. That is a graded requirement here. |
...
- **Verify:** register, then login, then `/me` with the token works. Bad creds return 401 with a clean error body.

### Part 4: Notes CRUD (backend)
- `notes` repository/service/controller
- `GET /api/notes` (search + pagination), `POST`, `GET /:id`, `PUT /:id`, `DELETE /:id`
- Ownership enforcement, so a user can only touch their own notes (404/403)
